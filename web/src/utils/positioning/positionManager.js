 ...
 * @param {Function} callback 位置更新回调函数
 */
export function setPositionUpdateCallback(callback) {
  if (typeof callback === 'function') {
    onPositionUpdateCallback = callback;
    return true;
  }
 ...
 */
export function getPositionHistory(limit = MAX_HISTORY_SIZE) {
 ...
 */
export function updatePosition(beacons) {
 ...
 * @returns {Object|null} 计算的位置对象
 */
export function calculatePositionByTrilateration(beacons) {
 ...
 * @returns {Object|null} 计算的位置对象
 */
export function calculatePositionByCentroid(beacons) {
  if (!Array.isArray(beacons) || beacons.length === 0) {
...
// Note: nonLinearLeastSquares, estimateAccuracy, smoothPosition, addToHistory are kept as internal helper functions.
// If they need to be used outside this module, they should also be exported. 